...
func Difficulte(w http.ResponseWriter, r *http.Request) {
	...
	template.Must(template.ParseFiles("templates/diff.html")).Execute(w, nil)
}
...
func (jeu *MonJeu) HangmanW(w http.ResponseWriter, r *http.Request) {
	if jeu.Word == "" { // Si la difficulté n'est pas définie, renvoyer à la page dédiée.
		http.Redirect(w, r, "/diff", http.StatusSeeOther)
	}
	if r.Method == http.MethodPost {
	...
	template.Must(template.ParseFiles("templates/hangman.html")).Execute(w, jeu)
}
...
func main() {
	...
	http.HandleFunc("/diff", Difficulte)
